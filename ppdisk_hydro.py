...
def getGasDensity(grid=None, ppar=None):
    ...
    fargodata = fp.read_fargo_data(path='/home/cas213/project/popsint_spiral_hp0p1_1mjup', frame=4, sigma=True, ir = grid.x/(50.0*au), iphi=grid.z)
    sigma1 = fargodata['isigma']

    z0 = np.zeros([grid.nx, grid.nz, grid.ny], dtype=np.float64)
    rho = np.zeros([grid.nx, grid.ny, grid.nz], dtype=np.float64)
    for iz in range(grid.nz):
        for iy in range(grid.ny):
            rho[:, iy, iz] = sigma1[iz,:] / (hp[:, iy, iz] * np.sqrt(2.0*np.pi)) * \
