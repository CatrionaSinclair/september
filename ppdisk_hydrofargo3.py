...
def getDustDensity(grid=None, ppar=None):
    ...
    d_x = grid.x[iix]
    print(grid.x.shape,d_x.shape,grid.z.shape)
    ...
    print(rc_dum[ixmin:].shape, d.phic.shape, d.sigmagas.T[:,ixmin:].shape)

    d.isigmagas[:,iix] = sigmagas_sp(d_x, grid.z)
    print(d.isigmagas.shape)
    
    alpha = (d.sigmagas[imin,0])*(rc_dum[imin])
    for iz in range(grid.nz):
	...
	dum1[:,iix] = sigmadust_sp(d_x,grid.z)
    	alpha = (dum[imin,0])*(rc_dum[imin])
    	for iz in range(grid.nz):
            for ix in range(grid.nx):
	    	if (np.isnan(dum1[iz,ix]) == True):
		    dum1[iz,ix] = alpha / grid.x[ix]
