...
def getDustDensity(grid=None, ppar=None):
    ...
    ynew = np.zeros([grid.nx,grid.nz,int(ngs)],dtype=float)
    st = np.zeros(d.Ndust+1, dtype=float)
    st_new = np.zeros([grid.nx, grid.nz, ppar['ngs']], dtype=float)

    st[1:] = np.array(d.stokesNr)
    st[0] = 1e-5
    print(st)

    for idust in range(len(gs)):
	print(idust)
	st_new[:,:,idust] = (np.pi/2.) * gdens * gs[idust] / d.isigmagas[iz,ix]
	for ix in range(grid.nx):
	    for iz in range(grid.nz):
  	    	y = np.array([d.isigmagas[iz,ix], dust0[iz,ix],dust1[iz,ix],dust2[iz,ix],dust3[iz,ix],dust4[iz,ix]])
		ynew[ix,iz,idust] = 10.**np.interp(np.log10(st_new[ix,iz,idust]), np.log10(st), np.log10(y))
