...
def getDustDensity(grid=None, ppar=None):
    ...
    alpha = (d.sigmagas[0:5,0].mean())*rc_dum[0]
    ...
    for iz in range(grid.nz):
		...
		if (grid.x[ix]<ppar['Rpl']):
		    ...
		    d.isigmagas[iz,ix] = alpha2 / grid.x[ix]
	...
	dum2 = d.sigmadust[i]
    	alpha = (dum2[0,:].mean())*(rc_dum[0])#(dum2[0:5,0].mean())*(rc_dum[0])
	alpha2 = (dum2[-1,:].mean())*(rc_dum[-1])#(dum2[-6:-1,0].mean())*(rc_dum[-1])
    	for iz in range(grid.nz):
            for ix in range(grid.nx):
